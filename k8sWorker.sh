...
main() {
    ...
    # Verify the API path before modifying this worker VM.
    python3 - "$CONTROL_PLANE_IP" <<'PY_TCP'
import socket
import sys
try:
    with socket.create_connection((sys.argv[1], 6443), timeout=10):
        print('Control-plane TCP/6443 is reachable.')
except OSError as exc:
    raise SystemExit(f'ERROR: Cannot reach {sys.argv[1]}:6443: {exc}. Finish control-plane setup and check cloud/host firewalls.')
PY_TCP

    start_logging
    prepare_node

    date -u +%FT%TZ > "$STATE_DIR/worker.prepared"
    log 'Worker prerequisites are installed. The worker has not joined yet.'
    cat <<EOF
On the CONTROL-PLANE VM, display:
  sudo cat $CONFIG_DIR/join-command.txt
Copy the displayed sudo kubeadm join command and execute it on THIS worker VM.
It registers this VM as "worker" and uses the private address $NODE_IP.

Until joining, kubelet may restart because kubeadm has not created its config.
After joining, run on the control plane:
  kubectl wait --for=condition=Ready node/worker --timeout=10m
  kubectl get nodes -o wide
  cilium status --wait --wait-duration 10m

The worker receives Cilium automatically from the cluster's DaemonSet.
Helm and the Cilium CLI are administration tools installed on the control plane.
EOF
}
